...
pub fn build(b: *std.Build) void {
    ...
    if (target.isDarwin()) {
        ...
        if (lib.isDynamicLibrary()) {
            const rc_command = b.addSystemCommand(&.{ resource_compiler, "windows/resources.rc" });
            const output = rc_command.addPrefixedOutputFileArg("-fo", "resources.res");
    ...
    for (examples, example_names) |example, name| {
        ...
        exe.linkLibrary(lib);
        if (target.isWindows()) {
            const rc_command = b.addSystemCommand(&.{ resource_compiler, "examples/resources.rc" });
            const output = rc_command.addPrefixedOutputFileArg("-fo", "resources.res");
            if (!is_dynamic) rc_command.addArg("-D_UI_STATIC");
            lib.addObjectFile(output);
        }
